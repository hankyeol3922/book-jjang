...
- ✅ **구글 로그인** — Supabase Auth 구글 OAuth(PKCE) 연동 · 프록시(미들웨어)로 세션 갱신 및 라우트 보호

실제 **구글 OAuth 로그인**으로 얻은 세션의 `auth.uid()`를 기준으로 RLS 정책이 그대로 작동합니다. 앱은 공개 키(anon)만 사용하고, RLS를 무시하는 `service_role` 키는 앱 코드에 등장하지 않습니다(초기 시드 스크립트에서만 1회성으로 사용).
